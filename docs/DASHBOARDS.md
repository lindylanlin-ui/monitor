...
- 目前有效封鎖 IP 總數、最近設定天數的 SSH 密碼／不存在帳號嘗試
- 保留 Nginx 日誌中的 `/file/` 登入失敗數，以及 collector 最後成功時間
- 各 jail 的目前封鎖數、短時間新增封鎖數與目前／累計計數
- SSH、Nginx 登入失敗趨勢
...
- `collector 距離上次成功` 超過 timer 間隔數倍時，應先依 [FAIL2BAN-MONITORING.md](./FAIL2BAN-MONITORING.md) 檢查 systemd 與 textfile 檔案。
...
- `TLS 憑證剩餘天數` 只會顯示 HTTPS 服務；少於 21 天持續 1 小時會觸發告警。
...
- 實體磁碟忙碌時間與所有本機檔案系統使用率
- NVIDIA GPU 使用率、溫度、功耗與已用顯示記憶體
- CPU 封裝／核心與 NVMe Composite 溫度

補充判讀：

- `SMART 健康狀態` 的 `正常` 代表磁碟整體 SMART 測試通過；`異常` 應立即確認備份與安排檢修。
